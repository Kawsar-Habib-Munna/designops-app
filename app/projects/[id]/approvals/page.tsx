...
// Screen 20 — Approvals (admin)। এডমিন approval আইটেম তৈরি করে ("Homepage Design"
// ইত্যাদি) — ক্লায়েন্ট Approve/Request Changes করলে এখানে স্ট্যাটাস আপডেট হয়ে যায়।
...
const STATUS_META: Record<string, { label: string; cls: string }> = {
  awaiting: { label: 'Awaiting Approval', cls: 's-review' },
  approved: { label: 'Approved ✓', cls: 's-done' },
  changes_requested: { label: 'Changes Requested', cls: 's-todo' },
};
...
export default function AdminApprovalsPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      const [projectRes, itemsRes, profileRes] = await Promise.all([
        ...
        supabase.from('client_approvals').select('id, item, status, comment, created_at').eq('project_id', projectId).order('created_at', { ascending: false }),
      ...
      setItems((itemsRes.data as Approval[]) ?? []);
  ...
  async function handleCreate(e: FormEvent) {
    e.preventDefault();
    if (!newItem.trim() || !project?.client_id) return;
    setCreating(true);
    await supabase.from('client_approvals').insert({ project_id: projectId, client_id: project.client_id, item: newItem.trim(), created_by: user!.id });
    setCreating(false);
    setNewItem('');
    setShowCreate(false);
    setReloadKey((k) => k + 1);
            ...
            {loading || !project ? (
              ...
              <>
                <div className="breadcrumb">
                  ...
                  <span className="sep">/</span>
                  <span className="current">Approvals</span>
                </div>
                <div className="proj-header">
                  <span className="proj-title">Approvals</span>
                  <div className="header-actions">
                    <button className="btn btn-accent btn-sm" onClick={() => setShowCreate(true)}>
                      <Icon name="plus" size={14} /> Request Approval
                    </button>
                  </div>
                </div>
                ...
                {items.length === 0 ? (
                  <div className="summary-card" style={{ textAlign: 'center', padding: '40px 20px' }}>
                    <p style={{ fontSize: 13, color: 'var(--ink-soft)' }}>এখনো কোনো approval request পাঠানো হয়নি।</p>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {items.map((a) => {
                      const meta = STATUS_META[a.status] ?? { label: a.status, cls: 's-todo' };
                      return (
                        <div className="summary-card" key={a.id} style={{ marginBottom: 0 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
                            <div>
                              <div style={{ fontSize: 14, fontWeight: 700 }}>{a.item}</div>
                              <div style={{ fontSize: 11, color: 'var(--ink-faint)' }}>{relativeTimeBn(a.created_at)}</div>
                            </div>
                            <span className={`status-pill ${meta.cls}`}>{meta.label}</span>
                          </div>
                          {a.comment && <p style={{ fontSize: 12.5, color: 'var(--ink-soft)', margin: '10px 0 0' }}>{a.comment}</p>}
                        </div>
                      );
                    })}
                  </div>
      ...
      {showCreate && (
        ...
        >
          <div className="modal-box">
            <div className="modal-title">Request Client Approval</div>
            <form onSubmit={handleCreate}>
              <label className="field-label">Item</label>
              <input className="field-input" type="text" value={newItem} onChange={(e) => setNewItem(e.target.value)} placeholder="যেমন: Homepage Design" autoFocus required />
              <div className="modal-foot">
                <button type="button" className="btn btn-ghost btn-sm" onClick={() => setShowCreate(false)}>
                  বাতিল
                </button>
                <button type="submit" className="btn btn-accent btn-sm" disabled={creating || !newItem.trim()}>
                  {creating ? 'পাঠানো হচ্ছে…' : 'Send Request'}
                </button>
              </div>
            </form>
          </div>
