...
// Screen 17 — Feedback (admin)। ক্লায়েন্টের ফিডব্যাক দেখা, স্ট্যাটাস বদলানো, আর
// "Convert to Task" চাপলে বিদ্যমান tasks টেবিলে একটা রিয়েল টাস্ক তৈরি হয় (এই
// প্রজেক্টের সাথে লিংক করা), workflow_stage='backlog' — Kanban/Task List-এ
// সাথে সাথে দেখা যাবে।
...
const STATUS_META: Record<string, { label: string; cls: string }> = {
  new: { label: 'New', cls: 's-review' },
  in_progress: { label: 'In Progress', cls: 's-review' },
  resolved: { label: 'Resolved', cls: 's-done' },
...
type Feedback = { id: string; title: string; description: string | null; attachment_url: string | null; status: string; converted_task_id: string | null; created_at: string };
...
export default function AdminFeedbackPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      const [projectRes, itemsRes, profileRes] = await Promise.all([
        ...
        supabase.from('client_feedback').select('id, title, description, attachment_url, status, converted_task_id, created_at').eq('project_id', projectId).order('created_at', { ascending: false }),
      ...
      setItems((itemsRes.data as Feedback[]) ?? []);
  ...
  async function updateStatus(id: string, status: string) {
    await supabase.from('client_feedback').update({ status, resolved_at: status === 'resolved' ? new Date().toISOString() : null }).eq('id', id);
    setReloadKey((k) => k + 1);
  }

  async function convertToTask(f: Feedback) {
    if (!user) return;
    setConvertingId(f.id);
    const { data: task, error } = await supabase
      .from('tasks')
      .insert({ title: f.title, description: f.description, project_id: projectId, status: 'todo', workflow_stage: 'backlog', created_by: user.id })
      .select('id')
      .single();
    if (!error && task) {
      await supabase.from('client_feedback').update({ status: 'in_progress', converted_task_id: task.id }).eq('id', f.id);
    }
            ...
            {loading || !project ? (
              ...
              <>
                ...
                <div className="proj-header">
                  <span className="proj-title">Client Feedback</span>
                </div>

                {items.length === 0 ? (
                  <div className="summary-card" style={{ textAlign: 'center', padding: '40px 20px' }}>
                    <p style={{ fontSize: 13, color: 'var(--ink-soft)' }}>এখনো কোনো ফিডব্যাক আসেনি।</p>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {items.map((f) => {
                      const meta = STATUS_META[f.status] ?? { label: f.status, cls: 's-todo' };
                      return (
                        <div className="summary-card" key={f.id} style={{ marginBottom: 0 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 8 }}>
                            <div>
                              <div style={{ fontSize: 14, fontWeight: 700 }}>{f.title}</div>
                              <div style={{ fontSize: 11, color: 'var(--ink-faint)' }}>{relativeTimeBn(f.created_at)}</div>
                            </div>
                            <span className={`status-pill ${meta.cls}`}>{meta.label}</span>
                          </div>
                          {f.description && <p style={{ fontSize: 12.5, color: 'var(--ink-soft)', margin: '0 0 10px' }}>{f.description}</p>}
                          {f.attachment_url && (
                            <a href={f.attachment_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent)', display: 'block', marginBottom: 10 }}>
                              View attachment ↗
                            </a>
                          )}
                          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            {f.status !== 'resolved' && (
                              <button className="btn btn-ghost btn-sm" onClick={() => updateStatus(f.id, 'resolved')}>
                                Mark Resolved
                              </button>
                            )}
                            {!f.converted_task_id && (
                              <button className="btn btn-accent btn-sm" onClick={() => convertToTask(f)} disabled={converting === f.id}>
                                {converting === f.id ? 'তৈরি হচ্ছে…' : 'Convert to Task'}
                              </button>
                            )}
                            {f.converted_task_id && <span style={{ fontSize: 11.5, color: 'var(--positive)', alignSelf: 'center' }}>✓ Converted to task</span>}
                          </div>
                        </div>
                      );
                    })}
