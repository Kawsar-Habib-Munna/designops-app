...
export default function ProjectsListPage() {
  ...
  async function handleCreate(e: FormEvent) {
    ...
    if (data) {
      ...
      setProjects((prev) => [...prev, row].sort((a, b) => (a.due_date ?? '9999').localeCompare(b.due_date ?? '9999')));
      ...
      <div className="shell">
        ...
        <div className="main">
          <header className="topbar">
            ...
            <button className="btn btn-accent" onClick={() => setShowCreate(true)}>+ নতুন তৈরি করুন</button>
          ...
          <main className="content">
            ...
            <p className="page-sub">সব প্রজেক্ট এক জায়গায় — বিস্তারিত দেখতে যেকোনো একটাতে ক্লিক করুন।</p>

            {error && (
              <div style={{ marginBottom: 16, padding: '12px 16px', borderRadius: 12, background: 'var(--danger-soft)', color: 'var(--danger)', fontSize: 13 }}>{error}</div>
            )}
            ...
            {loading ? (
              ...
              <div className="empty-state">
                <button className="empty-icon" onClick={() => setShowCreate(true)} aria-label="নতুন প্রজেক্ট তৈরি করুন">＋</button>
                <div className="empty-title">এখনও কোনো প্রজেক্ট নেই</div>
                <button className="btn btn-accent btn-sm" onClick={() => setShowCreate(true)} style={{ marginTop: 10 }}>+ প্রথম প্রজেক্ট তৈরি করুন</button>
              </div>
            ) : (
        ...
        >
          <div className="modal-box">
            ...
            <form onSubmit={handleCreate}>
              ...
              <input className="field-input" type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="যেমন: Aarambho — ব্র্যান্ড অ্যাপ রিডিজাইন" autoFocus required />

              <label className="field-label">ক্যাটাগরি</label>
              <input className="field-input" type="text" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} placeholder="যেমন: Mobile App · UX/UI (ঐচ্ছিক)" />
              ...
              <div className="field-row">
                <div>
                  ...
                  <input className="field-input" type="date" value={newStartDate} onChange={(e) => setNewStartDate(e.target.value)} />
                </div>
                <div>
              ...
              <input className="field-input" type="number" min="0" value={newBudget} onChange={(e) => setNewBudget(e.target.value)} placeholder="ঐচ্ছিক" />

              <label className="field-label">বিবরণ</label>
              <textarea className="field-input" rows={3} value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="প্রজেক্টের সংক্ষিপ্ত বিবরণ (ঐচ্ছিক)" style={{ resize: 'vertical', fontFamily: 'inherit' }} />

              <div className="modal-foot">
                <button type="button" className="btn btn-ghost btn-sm" onClick={() => setShowCreate(false)}>বাতিল</button>
                <button type="submit" className="btn btn-accent btn-sm" disabled={creating || !newName.trim()}>
